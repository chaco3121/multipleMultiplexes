package org.glassfish.movieplex7.json;

import java.io.IOException;
import java.io.InputStream;
import java.lang.annotation.Annotation;
import java.lang.reflect.Type;
import javax.json.Json;
import javax.json.stream.JsonParser;
import static javax.json.stream.JsonParser.Event.KEY_NAME;
import javax.ws.rs.Consumes;
import javax.ws.rs.WebApplicationException;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.MultivaluedMap;
import javax.ws.rs.ext.MessageBodyReader;
import javax.ws.rs.ext.Provider;
import org.glassfish.movieplex7.entities.Multiplex;

@Provider
@Consumes(MediaType.APPLICATION_JSON)
public class MultiplexReader implements MessageBodyReader<Multiplex> {

    @Override
    public boolean isReadable(Class<?> type, Type genericType,
            Annotation[] annotations, MediaType mediaType) {
        return Multiplex.class.isAssignableFrom(type);
    }

    @Override
    public Multiplex readFrom(Class<Multiplex> type, Type type1,
            Annotation[] annotations, MediaType mediaType,
            MultivaluedMap<String, String> multivaluedMap,
            InputStream inputSteam)
            throws IOException, WebApplicationException {
        Multiplex multiplex = new Multiplex();
        JsonParser parser = Json.createParser(inputSteam);

        while (parser.hasNext()) {
            switch (parser.next()) {
                case KEY_NAME:
                    String key = parser.getString();
                    parser.next();

                    switch (key) {
                        case "id":
                            multiplex.setId(parser.getString());
                            break;
                        case "city":
                            multiplex.setCity(parser.getString());
                            break;
                        default:
                            break;
                    }
                    break;
                default:
                    break;
            }
        }

        return multiplex;
    }
}
